    ...
    try {
      ...
      if (res.headers["x-powered-by"]) {
        findings.push({
          ...
          assumption:
            "It is harmless to tell attackers exactly what framework version we are running.",
        ...
        setCookie.forEach((cookie: string) => {
          ...
          if (!cookie.includes("SameSite")) {
            findings.push({
              ...
              assumption:
                "Browser defaults are fine (they are drifting towards Lax, but explicit is better).",
              reproduction: `Inspect cookies in DevTools`,
