...
export const raceConditionCheck: Check = {
  id: "race-condition",
  name: "Race Conditions",
  description:
    "Checks for lack of concurrency controls on state-changing endpoints",
  async run({ axios, apiAxios, discoveredRoutes, config }) {
    ...
    const candidates = discoveredRoutes.filter(
      (r) =>
        ...
        ["coupon", "vote", "gift", "claim", "transfer", "redeem"].some((k) =>
    ...
    for (const route of candidates) {
      ...
      try {
        ...
        const successes = statusCodes.filter(
        ...
        if (successes > 1) {
          ...
          findings.push({
            ...
            assumption:
              "Database transactions or logic are atomic without explicit locking.",
