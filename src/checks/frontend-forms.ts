  ...
  async run({ axios, apiAxios, discoveredRoutes }) {
    ...
    for (const route of htmlPages) {
      try {
        ...
        const validationTests: Promise<void>[] = [];
        ...
        $("form").each((_, form) => {
          ...
          const validatedInputs = $form.find(
            "input[required], input[pattern], input[min], input[max], input[minlength], input[maxlength], input[type='email'], input[type='url']",
          );
                ...
                try {
                  ...
                  if (res.status >= 200 && res.status < 300) {
                    findings.push({
                      ...
                      assumption:
                        "Client-side validation is enough; users can't bypass it.",
