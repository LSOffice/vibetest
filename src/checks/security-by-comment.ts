...
export const securityByCommentCheck: Check = {
  id: "security-by-comment",
  name: "Security by Comment (Intent Mismatch)",
  description:
    "Detects endpoints whose names imply security but are publicly accessible",
  async run({ axios, apiAxios, discoveredRoutes }) {
    ...
    for (const route of discoveredRoutes) {
      ...
      const mismatch = INTENT_MISMATCH_KEYWORDS.find((k) =>
        lowerPath.includes(k.keyword),
      );
