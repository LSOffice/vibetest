...
export const errorHandlingCheck: Check = {
  id: "error-handling",
  name: "Broken Error Handling & Info Leakage",
  description:
    "Triggers errors to check for leaked stack traces or database info",
  async run({ axios, apiAxios, discoveredRoutes }) {
    ...
    for (const route of testRoutes) {
      try {
        ...
        const res = await client.get(
          `${route.path}?id=NaN&page=-1&sort=INVALID`,
          { validateStatus: () => true },
        );
    ...
    function checkResponse(res: any, endpoint: string, vector: string) {
      if (res.status >= 400) {
        const body =
        ...
        for (const p of ERROR_PATTERNS) {
          if (p.pattern.test(body)) {
            findings.push({
              ...
              assumption:
                "Developer enabled verbose logging for debugging and forgot to disable it in this environment.",
