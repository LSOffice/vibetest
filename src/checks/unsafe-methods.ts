...
export const unsafeMethodsCheck: Check = {
  id: "unsafe-methods",
  name: "Unprotected State Change",
  description:
    "Checks for state-changing methods (POST/PUT/DELETE) that accept anonymous requests",
  async run({ axios, apiAxios, discoveredRoutes }) {
    ...
    for (const route of candidates) {
      ...
      if (SAFE_KEYWORDS.some((k) => lowerPath.includes(k))) continue;
      ...
      try {
        ...
        const isSuspicious =
          res.status < 300 || res.status === 400 || res.status === 500;
        ...
        if (isSuspicious) {
          ...
          findings.push({
            ...
            description: desc,
            assumption:
              "Developer assumed this specific method was protected because the route group is, or forgot it entirely.",
