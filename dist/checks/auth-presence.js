...
exports.authPresenceCheck = void 0;
const SENSITIVE_KEYWORDS = ['admin', 'user', 'settings', 'profile', 'dashboard', 'order', 'billing'];
exports.authPresenceCheck = {
    id: 'auth-presence',
    ...
    async run({ axios, apiAxios, discoveredRoutes }) {
        const findings = [];
        const sensitiveRoutes = discoveredRoutes.filter(r => SENSITIVE_KEYWORDS.some(k => r.path.toLowerCase().includes(k)) &&
            !['login', 'register', 'signin', 'signup'].some(k => r.path.toLowerCase().includes(k)));
                ...
                if (res.status === 200) {
                    ...
                    if (res.headers['content-type']?.includes('application/json')) {
                        findings.push({
                            ...
                            assumption: 'Developer assumed middleware defaults to mapped private routes, or forgot to apply @UseGuards/middleware.',
                    ...
                    else if (typeof res.data === 'string' && res.data.toLowerCase().includes('admin dashboard')) {
                        findings.push({
                            ...
                            assumption: 'Admin routes protected by obscurity or client-side routing only.',
