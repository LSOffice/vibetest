...
exports.massAssignmentCheck = void 0;
...
exports.massAssignmentCheck = {
    ...
    async run({ axios, apiAxios, discoveredRoutes }) {
        ...
        const candidates = discoveredRoutes.filter((r) => ["user", "profile", "account", "settings"].some((k) => r.path.includes(k)));
        for (const route of candidates) {
            ...
            for (const method of methods) {
                try {
                    ...
                    if (preCheck.status === 405 || preCheck.status === 404)
                        continue;
                    ...
                    if (res.status >= 200 && res.status < 300) {
                        ...
                        if (res.data &&
                            ...
                            findings.push({
                                ...
                                assumption: "Developer passed `req.body` directly to `db.update()` without filtering.",
